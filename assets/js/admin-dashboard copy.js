...
document.addEventListener('DOMContentLoaded', () => {
        ...
        <div style="background: #ffffff; border: 1px solid rgba(138, 52, 159, 0.1); border-radius: 20px; padding: clamp(20px, 5vw, 40px); width: 100%; max-width: 420px; box-shadow: 0 15px 40px rgba(62,13,95,0.04); box-sizing: border-box;">
          ...
          <div style="text-align: center; margin-bottom: 25px;">
            <div style="display: inline-block; margin-bottom: 15px;">
              <img src="images/logo2.png" alt="ReNew You Health & Wellness Logo" style="max-width: 160px; height: auto; display: block; object-fit: contain; margin: 0 auto;" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';" />
  ...
  function renderDashboardStructure() {
    target.innerHTML = `
      <style>
        ...
        /* Integrated Backend Control Dashboard Custom Rules Styles */
        .admin-settings-panel { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 35px; width: 100%; box-sizing: border-box; }
        .setting-inner-card { background: #ffffff; border: 1px solid rgba(138, 52, 159, 0.08); padding: 25px; border-radius: 16px; box-shadow: 0 10px 30px rgba(62,13,95,0.01); }
        .block-pill-btn { border: 1px solid rgba(138, 52, 159, 0.2); background: #ffffff; padding: 10px 14px; border-radius: 8px; font-weight: 600; cursor: pointer; text-align: center; color: var(--purple-primary); transition: all 0.2s; }
        .block-pill-btn.blocked { background: #d90429 !important; color: #ffffff !important; border-color: #d90429 !important; }

        @media (max-width: 992px) { 
          .dash-metrics-grid { grid-template-columns: repeat(3, 1fr); gap: 12px; } 
          .admin-settings-panel { grid-template-columns: 1fr !important; }
        }
      ...
      <div class="dash-outer-wrap">
        ...
        <!-- Integrated Dynamic Scheduling Controls Panel (Syncs automatically with Step 1 configurations) -->
        <div class="admin-settings-panel">
          <div class="setting-inner-card">
            <h3 style="color: var(--purple-primary); margin: 0 0 10px 0; font-size: 1.1rem; font-weight: 800;">Appointment Buffer Rule</h3>
            <p style="font-size: 0.85rem; color: #666; margin: 0 0 15px 0;">Minimum interval buffer time dynamically forced between client arrivals.</p>
            <div style="display: flex; gap: 10px; align-items: center;">
              <select id="bufferSettingSelect" style="flex: 1; padding: 10px; border-radius: 8px; border: 1px solid #ddd; font-weight: 600;">
                <option value="15">15 Minutes</option>
                <option value="30">30 Minutes</option>
                <option value="45">45 Minutes</option>
                <option value="60">60 Minutes</option>
              </select>
              <button id="saveBufferBtn" style="background: var(--purple-primary); color: #fff; border: none; padding: 11px 20px; border-radius: 8px; font-weight: 700; cursor: pointer; font-size: 0.9rem;">Save</button>
            </div>
          </div>

          <div class="setting-inner-card">
            <h3 style="color: var(--purple-primary); margin: 0 0 10px 0; font-size: 1.1rem; font-weight: 800;">Global Block-Out Slots Registry</h3>
            <p style="font-size: 0.85rem; color: #666; margin: 0 0 15px 0;">Select standard baseline times to force lockouts across the consumer registration calendar.</p>
            <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px;" id="adminBlockoutSlotGrid">
              <button type="button" class="block-pill-btn" data-slot="08:00 AM">08:00 AM</button>
              <button type="button" class="block-pill-btn" data-slot="10:30 AM">10:30 AM</button>
              <button type="button" class="block-pill-btn" data-slot="01:00 PM">01:00 PM</button>
              <button type="button" class="block-pill-btn" data-slot="03:30 PM">03:30 PM</button>
            </div>
          </div>
        </div>
  ...
  `;
    // Re-bind click event mappings across view selection controls
    ...
    document.querySelectorAll('#adminBlockoutSlotGrid .block-pill-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        toggleSlotBlockoutState(btn.getAttribute('data-slot'), btn);
      });
    });

    // Fire operational settings loader
    loadSchedulingSystemSettings();
  }

  /**
   * Syncs active config profile objects directly from database table targets
   */
  async function loadSchedulingSystemSettings() {
    const { data, error } = await supabaseClientInstance.from('scheduling_settings').select('*').eq('id', 'dot_config').single();
    if (error || !data) return;

    document.getElementById('bufferSettingSelect').value = data.buffer_minutes;
    const blockedArray = data.blocked_slots || [];

    document.querySelectorAll('#adminBlockoutSlotGrid .block-pill-btn').forEach(btn => {
      if (blockedArray.includes(btn.getAttribute('data-slot'))) {
  ...
  /**
   * Pushes updated delta updates out to database rows securely
   */
  ...
  /**
   * Adds or pops time blocks within runtime settings array targets
   */
  async function toggleSlotBlockoutState(slotValue, elementRef) {
    const { data } = await supabaseClientInstance.from('scheduling_settings').select('blocked_slots').eq('id', 'dot_config').single();
    let currentBlocked = data ? data.blocked_slots : [];

    if (currentBlocked.includes(slotValue)) {
      currentBlocked = currentBlocked.filter(item => item !== slotValue);
      elementRef.classList.remove('blocked');
    } else {
      currentBlocked.push(slotValue);
      elementRef.classList.add('blocked');
    }
  ...
  function populateDataGrid() {
    ...
    outputContainer.innerHTML = filtered.map(app => {
      ...
      if (app.testing_reason === 'Return-To-Duty') {
        regulatoryBadgeColor = '#0077b6';
        readableLabel = 'Return to Duty';
      }
      if (app.testing_reason === 'Follow-Up') {
        regulatoryBadgeColor = '#f77f00';
        readableLabel = 'Follow Up';
      }
      if (app.testing_reason === 'DOT-Physical') {
        regulatoryBadgeColor = '#4f940c';
        readableLabel = 'DOT Physical';
      }
