...
/**
 * Renders a modern healthcare email subscription capture banner
 */
/**
...
function renderSubscribeModule() {
    ...
    target.innerHTML = `
        ...
        <div style="background-color: var(--bg-white); padding: 20px 20px 20px; width: 100%; box-sizing: border-box; border-top: 1px solid rgba(138, 52, 159, 0.04); position: relative; overflow: hidden;">
            ...
            <div style="max-width: 1450px; margin: 0 auto;">
                ...
                <div style="background: var(--bg-offwhite); border: 1px solid rgba(138, 52, 159, 0.06); padding: 60px 50px; border-radius: 32px; box-shadow: 0 15px 45px rgba(62,13,95,0.01); display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; gap: 50px; box-sizing: border-box;">
                    
                    <!-- Left Side Column: Narrative Typography Block -->
                    <div style="flex: 1.2; min-width: 400px; text-align: left; box-sizing: border-box;">
                        <span style="color: var(--green-secondary); font-weight: 700; font-size: 0.82rem; text-transform: uppercase; letter-spacing: 2px; display: inline-block; background-color: rgba(79, 148, 12, 0.06); padding: 5px 14px; border-radius: 20px; margin-bottom: 16px;">
                            Stay Connected
                        </span>
                        
                        <h2 style="font-size: 2.2rem; color: var(--purple-primary); font-weight: 800; margin: 0 0 12px 0; letter-spacing: -0.5px; line-height: 1.2;">
                            Join Our Wellness Community
                        </h2>
                        
                        <p style="color: #555; font-size: 1.05rem; margin: 0; line-height: 1.6;">
                            Sign up to receive clinic announcements, preventative health insights, medical updates, and modern wellness advice delivered directly from our medical providers.
                        </p>
                    </div>

                    <!-- Right Side Column: Form Field Interaction Block -->
                    <div style="flex: 1; min-width: 380px; box-sizing: border-box;">
                        <form id="homeNewsletterForm" style="margin: 0;">
                            <div class="form-group" style="display: flex; gap: 12px; align-items: center; background-color: var(--bg-white); padding: 6px; border: 1px solid rgba(138, 52, 159, 0.15); border-radius: 50px; box-shadow: inset 0 2px 4px rgba(0,0,0,0.01);">
                                ...
                                <input type="email" id="newsletterEmail" placeholder="Enter your email address" required aria-label="Email address" 
                                    style="flex: 1; padding: 14px 20px; border: none; font-size: 1rem; outline: none; background: transparent; color: var(--purple-primary); box-sizing: border-box;">
                                
                                <!-- Modern Premium Pill Action Button -->
                                <button type="submit" id="newsletterSubmitBtn" 
                                    style="padding: 16px 28px; font-size: 1rem; font-weight: 700; border-radius: 50px; background: linear-gradient(135deg, var(--purple-primary), var(--purple-accent)); color: white; border: none; cursor: pointer; box-shadow: 0 6px 15px rgba(62,13,95,0.1); white-space: nowrap; box-sizing: border-box; transition: all 0.3s ease;">
                                    Subscribe Now
                                </button>
...
function setupNewsletterForm() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        /**
         * FIXED REST URL ENDPOINT
         * Table names with spaces must use %20 encoding for pure HTTP fetch requests.
         */
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Specific feedback for constraint: constraint leads_email_key unique (email)
